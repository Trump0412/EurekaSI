...
def aggregate(component, audit, merged):
    needed = ('ready_for_training', 'media_verified', 'six_sources_verified')
    complete = all(merged.get(k) is True for k in needed)
    try:
        validate_leakage_policy(merged)
    except ValueError:
        complete = False
    if merged.get('leakage_checked') is True and merged.get('global_scene_split_verified') is not True:
        complete = False
    complete = complete and all(Path(merged.get(k, '/nonexistent')).is_file() for k in ('train_manifest', 'validation_manifest'))
    ...
    return dict(status='prepared' if complete else 'pending_data_preparation',
                ...
                inventory_status=audit.get('status'), counts=audit.get('counts', {}),
